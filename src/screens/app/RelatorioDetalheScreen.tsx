...
const CAT_COLORS = ["#3B82F6","#8B5CF6","#F59E0B","#10B981","#EF4444","#EC4899","#14B8A6","#F97316","#6366F1","#84CC16"];
const PAY_COLORS = ["#3B82F6","#8B5CF6","#14B8A6","#F59E0B","#EF4444","#EC4899"];
const PERIOD_OPTIONS = [{ label: "1M", months: 1 },{ label: "3M", months: 3 },{ label: "6M", months: 6 },{ label: "12M", months: 12 }];

// ── Sub-components ────────────────────────────────────────────────────
function AnimBar({ ratio, color, delay = 0, maxH = 120 }: { ratio: number; color: string; delay?: number; maxH?: number }) {
...
function HBar({ ratio, color, delay = 0 }: { ratio: number; color: string; delay?: number }) {
  const anim = useRef(new Animated.Value(0)).current;
  useEffect(() => {
...
function EntradasSaidas({ items, months }: { items: FinanceItem[]; months: string[] }) {
...
function GastosCat({ items, months }: { items: FinanceItem[]; months: string[] }) {
...
function MaioresGastos({ items, months }: { items: FinanceItem[]; months: string[] }) {
...
function FluxoCaixa({ items, months }: { items: FinanceItem[]; months: string[] }) {
  let cumulative = 0;
...
// ── Report: Por conta ─────────────────────────────────────────────────
function PorConta({ items, months, accounts }: { items: FinanceItem[]; months: string[]; accounts: BankAccount[] }) {
  const filtered = items.filter(i => months.includes(isoToMonthKey(i.dateISO)));
  const totalDesp = filtered.filter(i => i.type === "DESPESA").reduce((s, i) => s + i.amountCents, 0);
...
// ── Report: Por cartão ────────────────────────────────────────────────
function PorCartao({ items, months, accounts }: { items: FinanceItem[]; months: string[]; accounts: BankAccount[] }) {
...
function ComparativoMeses({ items }: { items: FinanceItem[] }) {
...
function OrcamentoRealizado({ items, months }: { items: FinanceItem[]; months: string[] }) {
  const filtered = items.filter(i => months.includes(isoToMonthKey(i.dateISO)));
  const totalRec  = filtered.filter(i => i.type === "RECEITA").reduce((s, i) => s + i.amountCents, 0);
  const totalDesp = filtered.filter(i => i.type === "DESPESA").reduce((s, i) => s + i.amountCents, 0);
...
function DreSimplificado({ items, months }: { items: FinanceItem[]; months: string[] }) {
  const filtered  = items.filter(i => months.includes(isoToMonthKey(i.dateISO)));
  const totalRec  = filtered.filter(i => i.type === "RECEITA").reduce((s, i) => s + i.amountCents, 0);
  const totalDesp = filtered.filter(i => i.type === "DESPESA").reduce((s, i) => s + i.amountCents, 0);
...
function PorGrupo({ items, months }: { items: FinanceItem[]; months: string[] }) {
  const filtered = items.filter(i => months.includes(isoToMonthKey(i.dateISO)) && i.type === "DESPESA");
...
function RelatorioAnual({ items }: { items: FinanceItem[] }) {
...
function ExtratContador({ items, months }: { items: FinanceItem[]; months: string[] }) {
...
export default function RelatorioDetalheScreen() {
  ...
  return (
    <SafeAreaView style={c.root}>
      ...
      {/* Period — hidden for reports with their own selectors */}
